...
from ofa.utils import make_divisible, val2list

__all__ = ["CompOFAMobileNetV3"]


class CompOFAMobileNetV3(MobileNetV3):
    ...
    def __init__(
        ...
        self.width_mult = val2list(width_mult, 1)
        ...
        final_expand_width = [
            make_divisible(base_stage_width[-2] * max(self.width_mult), 8)
            for _ in self.width_mult
        ]
        last_channel = [
            make_divisible(base_stage_width[-1] * max(self.width_mult), 8)
            for _ in self.width_mult
        ]
        ...
        for base_width in base_stage_width[:-2]:
            width = [
                ...
                for width_mult in self.width_mult
            ]
        ...
        if len(set(input_channel)) == 1:
            first_conv = ConvLayer(
                3, max(input_channel), kernel_size=3, stride=2, act_func="h_swish"
            )
        ...
        super(CompOFAMobileNetV3, self).__init__(
    ...
    def load_weights_from_net(self, src_model_dict):
        ...
        for key in src_model_dict:
            ...
            ##############################################################################
                ...
                assert (
                    self.fixed_kernel
                ), f"The following key is expected in model dicts for both src & dst models, but is missing in dst model: {key}"
    ...
    def set_active_subnet(self, wid=None, ks=None, e=None, d=None, **kwargs):
        if self.fixed_kernel:
            assert (
                ks is None
            ), "You tried to set kernel size for a fixed kernel network!"
            ks = []
            kernel_stages = [3, 3, 5, 3, 3, 5]
            for k in kernel_stages[1:]:
                ks.extend([k] * 4)

        width_mult_id = val2list(wid, 4 + len(self.block_group_info))
    ...
    def get_active_net_config(self):
        ...
        if isinstance(self.first_conv, DynamicConvLayer):
            ...
            first_block_config = {
                ...
                "mobile_inverted_conv": self.blocks[
                    0
                ].mobile_inverted_conv.get_active_subnet_config(
                    first_conv_config["out_channels"]
                ),
                "shortcut": (
                    self.blocks[0].shortcut.config
                    if self.blocks[0].shortcut is not None
                    else None
                ),
            }
        ...
        if isinstance(self.classifier, DynamicLinearLayer):
            classifier_config = self.classifier.get_active_subnet_config(
                self.feature_mix_layer.active_out_channel
            )
        ...
        for stage_id, block_idx in enumerate(self.block_group_info):
            ...
            for idx in active_idx:
                middle_channel = make_divisible(
                    round(
                        input_channel
                        * self.blocks[idx].mobile_inverted_conv.active_expand_ratio
                    ),
                    8,
                )
                stage_blocks.append(
                    {
                        ...
                        "mobile_inverted_conv": {
                            ...
                            "out_channels": self.blocks[
                                idx
                            ].mobile_inverted_conv.active_out_channel,
                            "kernel_size": self.blocks[
                                idx
                            ].mobile_inverted_conv.active_kernel_size,
                            "stride": self.blocks[idx].mobile_inverted_conv.stride,
                            "expand_ratio": self.blocks[
                                idx
                            ].mobile_inverted_conv.active_expand_ratio,
                        ...
                        "shortcut": (
                            self.blocks[idx].shortcut.config
