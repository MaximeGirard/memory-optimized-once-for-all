...
__all__ = ["OFAMobileNetV3Ct"]


class OFAMobileNetV3Ct(MobileNetV3):
    def __init__(
        self,
        n_classes=1000,
        bn_param=(0.1, 1e-5),
        ...
        self.stride_stages = [1] + [2] * (self.n_stages - 2) + [1, 2]
        act_stages = ["relu"] + ["h_swish"] * (self.n_stages - 2) + ["relu", "h_swish"]
        se_stages = [False, False, True, False] + [True] * (self.n_stages - 3)
        n_block_list = [1] + [max(self.depth_list)] * self.n_stages # 1 stage of size 1 + 5 stages of size 4

        # Channel number between each stage
        initial_n_of_channels = 8
        
        base_stage_width = [3, initial_n_of_channels]
        for i in range(2, 2 + self.n_stages):
            base_stage_width.append(
                base_stage_width[-1] * (4 if self.stride_stages[i-1] == 2 else 1)
            )
                        
        final_expand_width = make_divisible(
            1024 * self.width_mult, MyNetwork.CHANNEL_DIVISIBLE
        )
        last_channel = make_divisible(
            2048 * self.width_mult, MyNetwork.CHANNEL_DIVISIBLE
        )
        ...
        super(OFAMobileNetV3Ct, self).__init__(
    ...
    def config(self):
        return {
            "name": OFAMobileNetV3Ct.__name__,
    ...
    def load_state_dict(self, state_dict, **kwargs):
        ...
        super(OFAMobileNetV3Ct, self).load_state_dict(model_dict)
